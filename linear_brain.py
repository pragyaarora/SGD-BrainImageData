...
def SgdHinge(X, Y, maxIter, learningRate, lmda):
    ...
    while iter<maxIter:
        ...
        for (xi, yi) in zip(X, Y):
            grad = HingeGradient(xi,yi,W,lmda)
            W -= learningRate*grad

        #Calculate new Loss and update old loss
...
def LogisticLoss(X, Y, W, lmda):
    loss = lmda * np.dot(W,W)
    loss += np.sum(np.logaddexp(0 , -Y * np.dot(X, W)))
    return loss

def LogisticGradient(x, y, W, lmda):
    grad = 2 * lmda * W
    e = y * np.dot(x, W)
    grad += (-y * x * (1/np.exp(np.logaddexp(0,e))))
    return grad
...
def RoisCal(W):
    W = np.delete(W, W.shape[0]-1) # remove bias
    W = np.reshape(W,data[1][0].shape)
    size = W.shape
    WNew = np.sum(W,axis=0)

    roi = rois[0]
    WRoi = np.zeros(roi.shape[0])
    ...
    for i in range(roi.shape[0]):
        ...
        for j in col:
            WRoi[i] += WNew[j-1]
        WRoi[i] /= float(col.size)

    indices = np.argsort(WRoi)[::-1]
    c = 1
    for i in indices:
        print c, ' ', str(roi[i]['name'][0]), ' ', WRoi[i]
        c += 1
...
def main():
    ...
    #         print "lmda :",y," eta :",x
    #         print "Logistic Loss"
    #         print "Accuracy (Logistic Loss):\t%s" % crossValidation(X, Y, SgdLogistic, maxIter=100, lmda=y, learningRate=x, sample=range(20))
    #         print "lmda :", y, " eta :", x
    #         print "Hinge Loss"
    #         print "Accuracy (Hinge Loss):\t%s" % crossValidation(X, Y, SgdHinge, maxIter=100, lmda=y, learningRate=x, sample=range(20))
    #         print "lmda :", y, " eta :", x
